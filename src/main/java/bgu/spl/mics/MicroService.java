...
public abstract class MicroService implements Runnable {
    ...
    private Map<Class<? extends Message>, Callback<? extends Message>> messageToAction = new HashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        messageToAction.put(type,callback); //adding this action to the event
        messageBus.subscribeEvent(type,this); //adding this MicroService to the event queue in MessageBus
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        messageToAction.put(type,callback); //adding this action to the Broadcast
        messageBus.subscribeBroadcast(type,this); //adding this MicroService to the Broadcast queue in MessageBus
    }
    ...
    public final void run() {
        ...
        while (!terminated) {
            try {
                Message message = messageBus.awaitMessage(this); //wait for a new message
                Class<? extends Message> messageClass = message.getClass();
                Callback currAction = messageToAction.get(messageClass); //get the action it should call
                if (currAction != null){
                    currAction.call(message);
                }
